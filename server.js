...
function getTeamLogoPath(match, teamKey) { // teamKey: 'TEAM1' или 'TEAM2'
    ...
    // Можно добавить нормализацию пути, если нужно (например, замена C:\ на относительный)
    return logoPath;
}
...
function getVRSResponse(matchId) {
    const rawVrsData = savedVRS[matchId] || defaultVrsStructure; // Получаем сырые данные или дефолт
    const match = savedMatches[matchId - 1] || defaultMatchStructure; // Получаем данные матча

    // Получаем пути к логотипам
    const team1Logo = getTeamLogoPath(match, 'TEAM1');
    const team2Logo = getTeamLogoPath(match, 'TEAM2');

    // Структура для пустого ответа (когда статус не FINISHED)
    const emptyFin = {
    ...
    // Структура для UPCOM/LIVE статуса
    const upcomData = {
        TEAM1: {
            winPoints: formatWinPoints(rawVrsData.TEAM1.winPoints),
            losePoints: rawVrsData.TEAM1.losePoints !== null ? -Math.abs(rawVrsData.TEAM1.losePoints) : "", // Гарантируем отрицательное или пустое
            rank: rawVrsData.TEAM1.rank ?? "",
            currentPoints: rawVrsData.TEAM1.currentPoints ?? "", // Отображаем текущие очки
            logo: team1Logo
        },
        TEAM2: {
    ...
    // Если матч завершен, формируем блок FINISHED
    if (match.FINISHED_MATCH_STATUS === "FINISHED") {
        ...
        // Если победитель не указан, finishedData останется emptyFin
    }
...
// --- API для VRS ---
// УДАЛЯЕМ GET /api/vrs-raw
// app.get("/api/vrs-raw", (req, res) => { res.json(savedVRS); });

// ВОЗВРАЩАЕМ GET /api/vrs/:id с обработкой
...
// PUT /api/vrs/:id - Обновить данные VRS для одного матча (остается без изменений)
app.put("/api/vrs/:id", async (req, res) => {
    ...
    // Сохраняем "сырые" данные, полученные от клиента
...
app.get("/api/teams", (req, res) => { /* ... (код без изменений) ... */ });
...
function gracefulShutdown() { /* ... (код без изменений) ... */ }
