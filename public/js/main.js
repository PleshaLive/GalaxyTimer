...
socket.on("mapVetoUpdate", (updatedMapVeto) => {
    ...
    updateMapVetoUI(updatedMapVeto); // Обновляем UI Veto
...
function updateMatchesUI(matches) {
  ...
  matches.forEach((match, index) => {
   ...
   const matchSelect = document.getElementById("matchSelect");
   if (matchSelect?.value && typeof updateVetoTeamOptions === 'function') { // Используем прямой вызов
       updateVetoTeamOptions(matchSelect.value); // Обновляем для текущего выбранного матча в Veto
   }
   // --- ---
...
function updateMapVetoUI(mapVetoData) {
    ...
    if (matchSelect && mapVetoData.matchIndex && matchSelect.value != mapVetoData.matchIndex) {
        ...
        if (typeof updateVetoTeamOptions === 'function') { // Используем прямой вызов
...
async function loadMapVetoFromServer() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const mapVetoData = await response.json(); // Ожидаем объект
...
async function saveMapVetoData(buttonElement) {
    ...
    setButtonState(buttonElement, 'saving'); // Устанавливаем состояние "сохранение"
    try {
        // Используем прямой вызов gatherMapVetoData из импортированного модуля
