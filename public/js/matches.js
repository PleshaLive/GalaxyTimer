...
export function gatherSingleMatchData(matchIndex) {
    ...
    // Автозаполнение счета карт ("NEXT", "DECIDER", "MATCH X") в зависимости от статуса
     if (statusText === "LIVE") {
        ...
        // Если первая карта сыграна (есть счет), а вторая нет (пустая или не счет)
        if (s1 && SCORE_REGEX.test(s1) && (!s2 || !SCORE_REGEX.test(s2))) {
            maps.MAP2_SCORE = "NEXT"; // Вторая карта следующая
            maps.MAP3_SCORE = "DECIDER"; // Третья - решающая
        }
        // Если две карты сыграны, а третья нет
    ...
    // Если статус матча не совпадает, поля для этого статуса будут пустыми или дефолтными
     const upcomObj = {
      ...
      UPCOM_next: "", // Это поле, кажется, больше не используется
    ...
    const liveObj = {
      ...
      LIVE_Cest: liveCestValue, // Путь к ongoing.png
      LIVE_VS: liveVs,
      LIVE_STATUS: liveStatusValue, // Путь к live.png
      LIVE_BG: liveBgValue, // Путь к LIVEBG.png
    ...
    const finishedObj = {
      ...
      FINISHED_TIME: statusText === "FINISHED" ? (timeVal ? timeVal + " CEST" : "") : "", // Добавляем CEST
    ...
    // Динамические логотипы уровня карт (показываются только если карта сыграна)
    const perMapLogos = {};
     [1, 2, 3].forEach(i => {
         const sc = maps[`MAP${i}_SCORE`];
         const isNum = SCORE_REGEX.test(sc); // Проверяем, является ли счет числовым
         // Показываем лого только если статус LIVE или FINISHED И счет числовой
         const show = (statusText === "LIVE" || statusText === "FINISHED") && isNum;
         // Используем ключи как в db.json для консистентности
         perMapLogos[`MAP${i}_TEAM1logo`] = show ? team1Logo : defaultLogo;
         perMapLogos[`MAP${i}_TEAM2logo`] = show ? team2Logo : defaultLogo;
     });

    // Динамические логотипы уровня матча (для FINISHED и LIVE)
    const matchLogos = {};
    const showFinishedLogos = statusText === "FINISHED";
    const showLiveLogos = statusText === "LIVE";
    // Генерируем ключи динамически для текущего матча 'm'
    ...
    // Собираем итоговый объект матча, объединяя все части
    const matchObj = {
      ...upcomObj,
      ...liveObj,
      ...finishedObj,
      MP1_UPC, MP2_UPC, MP3_UPC, // Иконки счета
      MP1_LIVE, MP2_LIVE, MP3_LIVE,
      MP1_FIN, MP2_FIN, MP3_FIN,
      Fin_cest: finCest,         // Текстовые поля для FINISHED
      FIN_Result: finResult,
      FIN_VICTORY: finVictory,
      TEAMWINNER: teamWinner,     // Имя и лого победителя
      TEAMWINNER_LOGO: teamWinnerLogo,
      ...matchLogos,              // Лого уровня матча
      ...perMapLogos               // Лого уровня карт
    };

    return matchObj; // Возвращаем собранный объект
}
...
function getScoreIcon(scoreStr) {
  // Проверка, что scoreStr это строка
  if (typeof scoreStr !== 'string') return "C:\\projects\\NewTimer\\files\\none.png";
  const parts = scoreStr.split(":"); // Разделяем счет по ':'
  ...
  // Сравниваем счет и возвращаем соответствующую иконку
  if (right > left) return "C:\\projects\\NewTimer\\files\\mp_R.png"; // Победила правая команда
  if (left > right) return "C:\\projects\\NewTimer\\files\\mp_L.png"; // Победила левая команда
  return "C:\\projects\\NewTimer\\files\\mp_none.png"; // Ничья или счет 0:0
}
