...
// --- КОНСТАНТЫ ДЛЯ ИКОНОК СЧЕТА КАРТ ---
const DEFAULT_MP_NONE_ICON = "C:\\projects\\NewTimer\\files\\mp_none.png";
const MP_L_ICON = "C:\\projects\\NewTimer\\files\\mp_L.png";
const MP_R_ICON = "C:\\projects\\NewTimer\\files\\mp_R.png";
...
// --- ЛОГИКА ДЛЯ ОПРЕДЕЛЕНИЯ ИКОНКИ СЧЕТА КАРТЫ (mp_none, mp_l, mp_r) ---
/**
 * Определяет путь к иконке счета на основе строки счета (например, "10:5").
 * @param {string} scoreStr - Строка счета.
 * @returns {string} Путь к файлу иконки.
 */
function getScoreIcon(scoreStr) {
  if (typeof scoreStr !== 'string') return DEFAULT_MP_NONE_ICON;
  const parts = scoreStr.split(":");
  if (parts.length !== 2) return DEFAULT_MP_NONE_ICON; // Некорректный формат

  const left = parseFloat(parts[0]);
  const right = parseFloat(parts[1]);

  // Если не числа (например, "NEXT", "DECIDER" или пустая строка), то none
  if (isNaN(left) || isNaN(right)) return DEFAULT_MP_NONE_ICON;

  if (right > left) return MP_R_ICON; // Правая команда выиграла карту
  if (left > right) return MP_L_ICON; // Левая команда выиграла карту
  return DEFAULT_MP_NONE_ICON; // Ничья или некорректный счет для определения победителя карты
}
...
function updateMatchesUI(matches) {
    ...
    matches.forEach((match, index) => {
        ...
        let prefix = ""; // Определяем префикс на основе основного статуса матча из данных
        ...
        mapRows.forEach((row, i) => {
            const mapIndexInRow = i + 1; // 1, 2, or 3
            ...
            const mapScoreIconElement = row.querySelector(".map-score-icon"); // Предполагаемый элемент для иконки счета карты
            ...
            const scoreValue = match[scoreKey]; // Это значение счета для текущего статуса, например, match.LIVE_MAP1_SCORE
            if (scoreInput) {
            ...
            // Обновление иконки счета карты (mp_none, mp_l, mp_r)
            if (mapScoreIconElement) {
                let iconSrc = DEFAULT_MP_NONE_ICON;
                if (prefix === "UPCOM_") {
                    iconSrc = DEFAULT_MP_NONE_ICON; // Для UPCOM всегда mp_none
                } else if (prefix === "LIVE_" || prefix === "FINISHED_") {
                    // scoreValue здесь - это актуальная строка счета для карты при данном статусе (LIVE/FINISHED)
                    // например, "16:10", "NEXT", ""
                    if (typeof scoreValue === 'string') { // Убедимся, что scoreValue это строка
                         iconSrc = getScoreIcon(scoreValue);
                    } else {
                         iconSrc = DEFAULT_MP_NONE_ICON; // Если scoreValue не строка или undefined
                    }
                }
                // Если prefix пустой (статус не определен), останется DEFAULT_MP_NONE_ICON
                mapScoreIconElement.src = iconSrc;
            }
        });
